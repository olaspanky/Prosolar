...
export const Hero = () => {
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      // Map the fields to Zoho's expected field names
      const zohoFormData = new FormData();
      zohoFormData.append('Last Name', values.lastName);
      zohoFormData.append('Phone', values.phone);
      zohoFormData.append('Email', values.email);
      zohoFormData.append('Description', values.more); // Map 'more' to 'Description'
      ...
      try {
        ...
        if (response.ok) {
          alert('Form submitted successfully to Zoho!');
          formik.resetForm();
        } else {
          alert('An error occurred while submitting the form to Zoho.');
        }
      } catch (error) {
        ...
        <div className="col-span-4 flex flex-col gap-5 lg:p-9 p-3 shadow-md border rounded-3xl">
          ...
          <div className="flex flex-col gap-5">
            <motion.div
              ...
              className="text-sm lg:text-lg xl:text-md lg:text-xl border-2 rounded-xl w-full shadow-md p-2 flex gap-2 justify-center items-center"
            >
